...
function defaultToXml(){
  return '';
}

function defaultAddHeaders(){
  return '';
}
...
function defaultPostProcess(xml){
  return xml;
}
...
Client.prototype.setSecurity = function() {
  var _this=this;
  var i;
  _this.security=[];
  _this.securityXML='';
  _this.securityHeaders={};
  _this.securityOptions={};

  if(arguments && arguments.length){
    for(i=0;i<arguments.length;i++){
      if(arguments[i].addHeaders){
        arguments[i].addHeaders(_this.securityHeaders);
      }
      if(arguments[i].addOptions){
        arguments[i].addOptions(_this.securityOptions);
      }
      if(arguments[i].toXML){
        this.securityXML+=arguments[i].toXML();
      }
      _this.signed=_this.signed||arguments[i].signer;
      arguments[i].postProcess=arguments[i].postProcess || defaultPostProcess;
      _this.security.push(arguments[i]);
    }

  }
  return _this.security;

};



Client.prototype.getJoinedSoapHeaders=function(){
  var headers=this.soapHeaders.join('\n')+this.securityXML;

  return (headers||this.signed)?('<soap:Header>'+headers+'</soap:Header>'):'';
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  for(i in this.securityOptions){
    options[i]= this.securityOptions[i];
  }

  // Add security options
  for(i in this.securityHeaders){
    headers[i]= this.securityHeaders[i];
  }
  ...
  self.security.forEach(function(securityMethod){
    if(securityMethod.postProcess){
      xml=securityMethod.postProcess(xml);
    }
  });
